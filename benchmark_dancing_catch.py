"""Standalone benchmark: DQN vs. a uniform-random agent on Dancing Catch.

Runs both agents over 500k timesteps for 30 seeds, giving 50 observation swaps,
then plots the reward EMA and the event-gated catch-rate EMA with 95% bootstrap
confidence bands and a guideline at each swap. DQN hyperparameters match
``benchmark_dqn.py`` so the two benchmarks are directly comparable.
    ...
    def step(carry, t):
        ...
        def do_train(params, opt_state):
            ...
            def loss_fn(p):
                ...
                # Only termination cuts the bootstrap; truncation keeps it.
...
def _compute_emas(rewards):
    ...
    def step(carry, reward):
        ...
        # Event-gated: only steps that resolved a ball update this EMA.
        resolved = reward != 0.0
...
def _mark_swaps(ax, swap_steps):
    """Draw a faint vertical guideline at each swap step."""
    for step in swap_steps:
        ax.axvline(step, color="gray", alpha=0.15, lw=0.8, zorder=0)
...
def make_plot(dqn_rewards, random_rewards, path):
    ...
    # Downsampling keeps bootstrap_mean_ci's intermediate arrays small. The
    # final index is TOTAL_TIMESTEPS - 1, so the summary reports the true value.
    ...
    swap_steps = range(SWAP_EVERY, TOTAL_TIMESTEPS + 1, SWAP_EVERY)
    _mark_swaps(ax_r, swap_steps)
    ...
    # A darker proxy gives the guidelines one legible legend entry.
...
def main():
    ...
    make_plot(results["DQN"], results["Random"], "benchmark_dancing_catch.pdf")
